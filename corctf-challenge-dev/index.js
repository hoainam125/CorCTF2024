...
app.get("/submit", (req, res) => {
    ...
    if (process.env.ORIGIN && urlObj.origin !== process.env.ORIGIN) {
        return res.send(`you can only submit URLs from this origin: ${process.env.ORIGIN}`);
    }

    const deltaTime = +new Date() - (req.session.lastVisit || 0);
    if (deltaTime < 30_000) {
        return res.send(`please slow down (wait ${(30_000 - deltaTime)/1000} more seconds)`);
    }
